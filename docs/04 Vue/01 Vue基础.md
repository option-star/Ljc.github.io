...
​	watch方法原理内部会通过创建一个`watcher`实例的方法，进行数据的依赖收集，当依赖变化的时候会调用用户传入的方法。

​	当`deep`属性设置为`true`时，表示进行深度观察。只需要在创建`watcher`实例的时候把`obj`对象所有的值都递归读一遍，那么这个`watcher`实例就会被加到对象内所有值的依赖列表中，之后当对象任意某个值发生变化时就能得到通知。
