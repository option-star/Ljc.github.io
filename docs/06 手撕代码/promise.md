...
1. 先建立一个存储promise执行结果的数组： result
2. 然后设置判断是否运行完的变量： count
3. 通过Promise.resolve.then的方法循环调用传入数组的每一项，并设置设置结果入result
4. 直至count的值与传入数组长度一致后通过return resolve(result)返回
